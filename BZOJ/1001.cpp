...
int main() {
    ...
    gao->init(n * m, 0, n * m - 1);
    for(int i = 0; i < n; i++) {
        for(int j = 1, tmp; j < m; j++) {
            scanf("%d", &tmp);
            gao->add_edge(i * m + j - 1, i * m + j, tmp);
            gao->add_edge(i * m + j, i * m + j - 1, tmp);
        }
    }
    for(int i = 1; i < n; i++) {
        for(int j = 0, tmp; j < m; j++) {
            scanf("%d", &tmp);
            gao->add_edge((i - 1) * m + j, i * m + j, tmp);
            gao->add_edge(i * m + j, (i - 1) * m + j, tmp);
        }
    }
    for(int i = 1; i < n; i++) {
        for(int j = 1, tmp; j < m; j++) {
            scanf("%d", &tmp);
            gao->add_edge((i - 1) * m + j - 1, i * m + j, tmp);
            gao->add_edge(i * m + j, (i - 1) * m + j - 1, tmp);
        }
    }
    printf("%d\n",gao->dinic());
    return 0;
